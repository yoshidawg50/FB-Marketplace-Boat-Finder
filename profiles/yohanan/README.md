# Yohanan's Profile

Yohanan's rods/reels; the universal knowledge lives in the branch notes and
every entry here links out to it. The day-plan protocol resolves
recommendations against this profile (owned gear) and falls back to class
terms for anything not yet filled in.

- [rods.md](rods.md) — the 4 rod/reel setups with line classes.
- [boat.md](boat.md) — not yet filled in.
- [tackle.md](tackle.md) — not yet filled in.
- [trolling-lures.md](trolling-lures.md) — not yet filled in.
- [spots.md](spots.md) — not yet filled in.

<!-- index:start -->
## Index

- [<Your Boat>](boat.md) — Operating envelope + trolling platform.
- [Yohanan's Rods — 4 Setups](rods.md) — Rod/reel inventory.
- [<Your> Spots](spots.md) — Home waters and (optionally) named spots.
- [<Your> Tackle — Casting & Jigging (owned)](tackle.md) — Your owned casting/jigging hard baits.
- [<Your> Trolling Lures (owned)](trolling-lures.md) — Your owned trolling spread.
<!-- index:end -->

<!-- mermaid:start -->
## Map

_No intra-folder links yet._
<!-- mermaid:end -->
